...
/**
 * HP Ledger — Hire Purchase (flat EMI).
...
import { calcHP, formatDate, getEffectiveRule as getLedgerRule } from '@/lib/finance'
...
export default function HPLedgerPage() {
  ...
  const calc = useMemo(() => {
    ...
    const tenureMonths = Math.max(1, Math.round((Number(loan.period) || 30) / 30))
    ...
    return calcHP({
      ...
      tenureMonths, installments: tenureMonths,
      amountPaid,
    })
  ...
  return (
    <div>
      <PageHeader
        title="HP Ledger"
        subtitle="Hire Purchase · flat-rate EMI, fixed tenure"
        breadcrumbs={[{ label: 'Dashboard', href: '/' }, { label: 'Ledgers' }, { label: 'HP' }]}
        actions={
          <>
      ...
      <div className="p-6 grid gap-6 lg:grid-cols-[320px_1fr]">
        ...
        <div className="space-y-6">
          {!selected ? (
            <Card><CardBody><EmptyState title="Select an HP account" description="Pick an account to see EMI schedule and totals." /></CardBody></Card>
          ) : (
            <>
              <Card>
                <CardHeader title={loan.customerName || '—'} subtitle={`HP #${loan.number} · ${formatDate(loan.date)}`} actions={<Badge tone="info">HP</Badge>} />
                <CardBody>
                ...
                <Card>
                  ...
                  <CardBody>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <Field label="Rate (% / month flat)"><Input type="number" step="0.01" value={loan.rateOfInterest ?? rule.defaultRate} onChange={e => setLoan({ ...loan, rateOfInterest: Number(e.target.value) })} /></Field>
                  ...
                  <CardBody>
                    {!calc ? <p className="text-sm text-slate-500">Enter principal and date.</p> : (
                      <InfoGrid columns={2} items={[
                        ...
                        { label: 'Installments',   value: calc.installments },
                        { label: 'EMI',            value: <Money value={calc.emi} /> },
                        ...
                        { label: 'Outstanding',    value: <Money value={calc.outstanding} tone="debit" /> },
                      ]} />
                    )}
                  </CardBody>
                </Card>
              ...
              <Card>
                <CardHeader title="EMI Schedule" subtitle={`${calc?.installments || 0} installments`} />
                <CardBody className="!p-0">
                  {!calc ? <div className="p-5 text-sm text-slate-500">—</div> : (
                    <DataTable className="!border-0 !rounded-none">
                      <thead>
                        <tr><th>#</th><th>Due date</th><th className="text-right">Principal</th><th className="text-right">Interest</th><th className="text-right">EMI</th><th className="text-right">Balance</th></tr>
                      </thead>
                      <tbody>
                        {calc.schedule.map(r => (
                          <tr key={r.sn}>
